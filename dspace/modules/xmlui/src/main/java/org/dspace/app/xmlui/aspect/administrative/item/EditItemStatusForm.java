...
public class EditItemStatusForm extends AbstractDSpaceTransformer {
	...
	private static final Message T_label_doi = message("xmlui.administrative.item.EditItemStatusForm.label_doi");
	...
	public void addBody(Body body) throws SQLException, WingException
	{
		...
		itemInfo.addLabel(T_label_doi);
		String itemDoi = DOIIdentifierProvider.getDOIByObject(context, item);
		if (itemDoi != null && !itemDoi.isEmpty()) {
		    itemInfo.addItem(itemDoi);
		} else {
		    if (AuthorizeManager.authorizeActionBoolean(context, item, Constants.WRITE))
	        {
		        itemInfo.addItem().addButton("submit-create-doi").setValue(T_submit_register_doi);
	        }
	        else
	        {
	            addNotAllowedButton(itemInfo.addItem(), "submit-create-doi", T_submit_register_doi);
	        }
		}
